...
void GameApp::OnInitialize()
{
	...
	mUIPixelShader.Initialize(L"../Assets/Shaders/DepthMap.fx", "PSDebug", "ps_5_0");
	...
	random = rand();
	mConstantBufferNode1 = mSceneManager->CreateConstantBufferNode(0);
	staticEffectShaderValues = { mTimer.GetElapsedTime(), 0.009f, (float)random, 1.0f };
	mConstantBufferNode1->UpdateData(staticEffectShaderValues);

	mConstantBufferNode2 = mSceneManager->CreateConstantBufferNode(1);
	...
	mModelNode1 = mSceneManager->CreateModelNode("Trex.wow");
	mModelNode1->SetName("TrexModel");
	mModelNode1->GetTransformNode().GetTransform().SetPosition({ 20.0f, 0.0f, -5.0f });
	mModelNode1->GetTransformNode().GetTransform().SetScale({ 5.0f, 5.0f, 5.0f });

	mModelNode2 = mSceneManager->CreateModelNode("Naruto.wow");
	mModelNode2->SetName("NarutoModel");
	mModelNode2->GetTransformNode().GetTransform().SetPosition({ -20.0f, 0.0f, -5.0f });
	mModelNode2->GetTransformNode().GetTransform().SetRotation({ 90.0f, 0.0f, 0.0f });
	mModelNode2->GetTransformNode().GetTransform().SetScale({ 0.2f, 0.2f, 0.2f });

	mModelNode3 = mSceneManager->CreateModelNode("SamsungTV.wow");
	...
	mModelNode4 = mSceneManager->CreateModelNode("OldHouse.wow");
	...
	mModelNode5 = mSceneManager->CreateModelNode("Building_01.wow");
...
void GameApp::OnUpdate()
{
	...
	staticEffectShaderValues.z = (float)random;
	mConstantBufferNode1->UpdateData(staticEffectShaderValues);
	mConstantBufferNode2->UpdateData({ shaderMode, 0.0f, 0.0f, 0.0f });
